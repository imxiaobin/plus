...
def _test_sms(driver_type: str, extra: dict) -> dict:
    """查询接码平台余额，验证 API Key 是否可用。"""
    import traceback

    if driver_type != "herosms":
        return {"ok": False, "error": f"不支持测试的短信平台: {driver_type}"}
    from providers.sms.herosms import HeroSMSClient, HeroSMSError

    try:
        client = HeroSMSClient.from_config(extra)
        balance = client.get_balance()
        return {"ok": True, "message": f"连接成功，余额: {balance}"}
