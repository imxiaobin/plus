...
class ApiExtractProvider(BaseProxyProvider):
    ...
    def _normalize(self, raw: str) -> str:
        ...
        from core.proxy_url import to_proxy_url

        raw = raw.strip()
        parsed = to_proxy_url(raw, default_scheme=self.protocol)
        if parsed:
            if self.username and self.password and "@" not in parsed.split("://", 1)[-1]:
                hostport = parsed.split("://", 1)[-1]
                return f"{self.protocol}://{self.username}:{self.password}@{hostport}"
            return parsed
