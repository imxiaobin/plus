...
class PhoneIdentityProvider(BaseIdentityProvider):
    ...
    def resolve(self, requested_email: Optional[str] = None) -> IdentityMaterial:
        ...
        from providers.sms.herosms import HeroSMSClient, normalize_e164

        client = HeroSMSClient.from_config(self.extra)
        if requested:
            phone = normalize_e164(requested)
            return IdentityMaterial(
                identity_provider=self.identity_provider,
                email=phone,
                metadata={
                    ...
                    "activation_id": str(self.extra.get("herosms_activation_id") or ""),
                },
            )
        ...
        return IdentityMaterial(
            identity_provider=self.identity_provider,
            email=activation.phone,
            metadata={
                "phone": activation.phone,
                "sms_provider": "herosms",
                "activation_id": activation.activation_id,
                "herosms_service": client.service,
                "herosms_country": client.country,
            },
        )
