...
import HttpProxyPoolSettings from "@/pages/HttpProxyPoolSettings";
import Sub2ApiSettings from "@/pages/Sub2ApiSettings";
...
export default function SettingsPage({
  ...
  const tab = ["general", "mailbox", "proxy-pool", "http-proxy-pool", "sms-platform", "sub2api"].includes(requestedTab)
    ? requestedTab
    : "general";
  ...
  const titles: Record<string, string> = {
    ...
    "http-proxy-pool": t("settings.title.httpProxyPool"),
    "sms-platform": t("settings.title.smsPlatform"),
    sub2api: t("settings.title.sub2api"),
  };
  const mobileTabs = [
    ...
    { value: "http-proxy-pool", label: t("nav.settings.httpProxyPool") },
    { value: "sms-platform", label: t("nav.settings.smsPlatform") },
    { value: "sub2api", label: t("nav.settings.sub2api") },
  ];
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {tab === "http-proxy-pool" && <HttpProxyPoolSettings />}
      {tab === "sms-platform" && <Settings kind="sms" />}
      {tab === "sub2api" && <Sub2ApiSettings />}
    </div>
  );
}
